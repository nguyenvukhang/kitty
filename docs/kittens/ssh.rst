...
* Make the kitten binary available in the remote host :opt:`on demand <kitten-ssh.remote_alatty>`

* Easily :opt:`change terminal colors <kitten-ssh.color_scheme>` when connecting to remote hosts

* Automatically :opt:`forward the alatty remote control socket <kitten-ssh.forward_remote_control>` to configured hosts
...
alatty terminfo database there.
...
If you define a mapping in :file:`alatty.conf` such as::
...
The ssh kitten can be configured using the :file:`~/.config/alatty/ssh.conf` file
...
.. code-block:: conf
   ...
   env COPIED_VAR=_alatty_copy_env_var_
...
reads setup data over the TTY device, which alatty sends as a Base64 encoded
...
the kitten over the TTY with a random one time password. alatty reads the request
...
.. note::
   ...
   environment variables :envvar:`ALATTY_PID` and :envvar:`ALATTY_WINDOW_ID` are
...
following one liner::
    ...
    infocmp -a xterm-alatty | ssh myserver tic -x -o \~/.terminfo /dev/stdin
...
:file:`~/.terminfo/x/xterm-alatty`.

If the server is running a relatively modern Linux distribution and you have
root access to it, you could simply install the ``alatty-terminfo`` package on
...
machine by running (on local machine with |alatty|)::

    infocmp -CrT0 xterm-alatty
