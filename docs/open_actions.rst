...
|alatty| has support for :term:`terminal hyperlinks <hyperlinks>`. These are
...
these hyperlinks in |alatty|.

You can tell alatty to take arbitrarily many, complex actions when a link is
clicked. Let us illustrate with some examples, first. Create the file
:file:`~/.config/alatty/open-actions.conf` with the following:

.. code:: conf

    # Open any image in the full alatty window by clicking on it
    protocol file
    mime image/*
    action launch --type=overlay kitten icat --hold ${FILE_PATH}

Now, run ``ls --hyperlink=auto`` in alatty and click on the filename of an
...
``action`` entries. In the example above alatty uses the :doc:`launch <launch>`
...
:file:`alatty.conf` can be used as an action. You can specify more than one
...
``EDITOR``
    ...
    :file:`alatty.conf` is preferred.

``SHELL``
    The path to the shell. The configured :opt:`shell` in :file:`alatty.conf` is
    preferred, without arguments.

.. note::
   You can use the :opt:`action_alias` option just as in :file:`alatty.conf` to
...
``mime``
    ...
    application/pdf``. You can add MIME types to alatty by creating a file named
    :file:`mime.types` in the :ref:`alatty configuration directory <confloc>`.
...
Scripting the opening of files with alatty on macOS
-------------------------------------------------------

On macOS you can use :guilabel:`Open With` in Finder or drag and drop files and
URLs onto the alatty dock icon to open them with alatty. The default actions are:
...
These actions can also be executed from the command line by running::
    ...
    alatty +open file_or_url another_url ...

    # macOS only
    open -a alatty.app file_or_url another_url ...

Since macOS lacks an official interface to set default URL scheme handlers,
alatty has a command you can use for it. The first argument for is the URL
scheme, and the second optional argument is the bundle id of the app, which
defaults to alatty, if not specified. For example:

.. code-block:: sh

    # Set alatty as the handler for ssh:// URLs
    alatty +runpy 'from alatty.fast_data_types import cocoa_set_url_handler; import sys; cocoa_set_url_handler(*sys.argv[1:]); print("OK")' ssh
    # Set someapp as the handler for xyz:// URLs
    alatty +runpy 'from alatty.fast_data_types import cocoa_set_url_handler; import sys; cocoa_set_url_handler(*sys.argv[1:]); print("OK")' xyz someapp.bundle.identifier

You can customize these actions by creating a :file:`launch-actions.conf` file
in the :ref:`alatty config directory <confloc>`, just like the
:file:`open-actions.conf` file above. For example:

.. literalinclude:: ../alatty/open_actions.py
   :language: conf
