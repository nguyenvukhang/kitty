...
import alatty.fast_data_types as fast_data_types

from .constants import handled_signals, is_freebsd, is_macos, kitten_exe, alatty_base_dir, shell_path, terminfo_dir
...
if is_macos:
    from alatty.fast_data_types import cmdline_of_process as cmdline_
    from alatty.fast_data_types import cwd_of_process as _cwd
    from alatty.fast_data_types import environ_of_process as _environ_of_process
    from alatty.fast_data_types import process_group_map as _process_group_map
...
def process_env() -> Dict[str, str]:
    ans = dict(os.environ)
    ssl_env_var = getattr(sys, 'alatty_ssl_env_var', None)
...
class Child:
    ...
    def get_final_env(self) -> Dict[str, str]:
        ...
        if tdir:
            env['TERMINFO'] = tdir
        env['ALATTY_INSTALLATION_DIR'] = alatty_base_dir
        ...
        if self.is_clone_launch:
            ...
            env.pop('ALATTY_IS_CLONE_LAUNCH', None)
        return env
    ...
    def fork(self) -> Optional[int]:
        ...
        if self.should_run_via_run_shell_kitten:
            ...
            # https://github.com/kovidgoyal/alatty/issues/247
            ...
            # https://github.com/kovidgoyal/alatty/issues/1870
            ...
            if is_macos:
                ...
                # https://github.com/kovidgoyal/alatty/issues/6511
                import pwd
