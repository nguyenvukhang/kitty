...
#define ALATTY_HANDLED_SIGNALS SIGINT, SIGHUP, SIGTERM, SIGCHLD, SIGUSR1, SIGUSR2, 0
...
static PyObject*
mask_alatty_signals_process_wide(PyObject *self UNUSED, PyObject *a UNUSED) {
    mask_variadic_signals(0, ALATTY_HANDLED_SIGNALS);
    Py_RETURN_NONE;
}
...
new(PyTypeObject *type, PyObject *args, PyObject UNUSED *kwds) {
    ...
    if (!init_loop_data(&self->io_loop_data, ALATTY_HANDLED_SIGNALS)) return PyErr_SetFromErrno(PyExc_OSError);
...
render_prepared_os_window(OSWindow *os_window, unsigned int active_window_id, color_type active_window_bg, unsigned int num_visible_windows, bool all_windows_have_same_bg) {
    ...
    br->is_dirty = false;
    if (TD.screen && os_window->num_tabs >= OPT(tab_bar_min_tabs)) draw_cells(TD.vao_idx, &TD, os_window, true, false);
    for (unsigned int i = 0; i < tab->num_windows; i++) {
        Window *w = tab->windows + i;
        if (w->visible && WD.screen) {
            bool is_active_window = i == tab->active_window;
            draw_cells(WD.vao_idx, &WD, os_window, is_active_window, true);
...
no_render_frame_received_recently(OSWindow *w, monotonic_t now, monotonic_t max_wait) {
    return now - w->last_render_frame_received_at > max_wait;
}
...
thread_write(void *x) {
    ...
    set_thread_name("AlattyWriteStdin");
...
#ifdef ALATTY_PRINT_BYTES_SENT_TO_CHILD
static void
...
io_loop(void *data) {
    ...
    set_thread_name("AlattyChildMon");
