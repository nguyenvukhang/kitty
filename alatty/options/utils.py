...
import alatty.fast_data_types as defines
...
from alatty.utils import expandvars, log_error, resolve_abs_or_config_path, shlex_split
...
mod_map = {'⌃': 'CONTROL', 'CTRL': 'CONTROL', '⇧': 'SHIFT', '⌥': 'ALT', 'OPTION': 'ALT', 'OPT': 'ALT',
           '⌘': 'SUPER', 'COMMAND': 'SUPER', 'CMD': 'SUPER', 'ALATTY_MOD': 'ALATTY'}
...
@func_with_args('goto_layout', 'toggle_layout', 'alatty_shell', 'set_tab_title', 'push_keyboard_mode')
...
def parse_layout_names(parts: Iterable[str]) -> List[str]:
    from alatty.layout.interface import all_layouts
    ans = []
    for p in parts:
        p = p.lower()
        if p in ('*', 'all'):
...
def resolve_key_mods(alatty_mod: int, mods: int) -> int:
    return SingleKey(mods=mods).resolve_alatty_mod(alatty_mod).mods
...
class MouseMapping(BaseDefinition):
    ...
    def resolve_and_copy(self, alatty_mod: int) -> 'MouseMapping':
        ans = MouseMapping(
            self.button, resolve_key_mods(alatty_mod, self.mods), self.repeat_count, self.grabbed,
            self.definition)
...
class KeyDefinition(BaseDefinition):
    ...
    def resolve_and_copy(self, alatty_mod: int) -> 'KeyDefinition':
        def r(k: SingleKey) -> SingleKey:
            return k.resolve_alatty_mod(alatty_mod)
        ans = KeyDefinition(
