...
opt('force_ltr', 'no',
    ...
    long_text='''
alatty does not support BIDI (bidirectional text), however, for RTL scripts,
words are automatically displayed in RTL. That is to say, in an RTL script, the
words "HELLO WORLD" display in alatty as "WORLD HELLO", and if you try to select
...
writes into the selection buffer the character י. alatty's default behavior is
...
will force alatty to always treat the text as LTR, which FriBidi expects for
terminals.
'''
    )
...
opt('text_composition_strategy', 'platform',
    ...
    long_text='''
Control how alatty composites text glyphs onto the background color. The default
value of :code:`platform` tries for text rendering as close to "native" for
the platform alatty is running on as possible.

A value of :code:`legacy` uses the old (pre alatty 0.28) strategy for how glyphs
...
color, below which alatty will override the foreground color. It is percentage
...
:code:`filter`:
    ...
    the alatty config directory on the pasted text. The text returned by the
...
You can run alatty with the :option:`alatty --debug-input` command line option
...
the entire screen on each loop, because alatty is so fast that partial screen
...
new instances of alatty will have the same size as the previous instance.
...
opt('window_resize_step_cells', '2',
    ...
    long_text='''
The step size (in units of cell width/cell height) to use when resizing alatty
...
opt('resize_debounce_time', '0.1 0.5',
    ...
    long_text='''
The time to wait (in seconds) before asking the program running in alatty to resize and
...
redraw-after-pause since alatty can distinguish between a pause and end of
resizing. On such systems the first number is ignored and redraw is immediate
after end of resize. On other systems only the first number is used so that alatty
...
number of alatty windows in it by window manager (e.g. clicking the window close
...
:code:`custom`
    ...
    alatty's source code: :file:`alatty/tab_bar.py`. See also
...
alatty config and not use a background color in the editor color scheme. Or use
...
bright version, for the first 16 colors.
''')
...
use when editing the alatty config file or similar tasks.
...
set, alatty will run your :opt:`shell` (:code:`$SHELL -l -i -c env`) to see if
your shell startup rc files set :envvar:`VISUAL` or :envvar:`EDITOR`. If that
doesn't work, alatty will cycle through various known editors (:program:`vim`,
...
alatty window. Relative paths are resolved relative to the alatty config
...
opt('+exe_search_path', '',
    ...
    add_to_default=False,
    long_text='''
Control where alatty finds the programs to run. The default search order is:
...
opt('startup_session', 'none',
    ...
    long_text='''
Path to a session file to use for all alatty instances. Can be overridden by
using the :option:`alatty --session` :code:`=none` command line option for individual
instances. See :ref:`sessions` in the alatty documentation for details. Note that
relative paths are interpreted with respect to the alatty config directory.
...
opt('clipboard_control', 'write-clipboard write-primary read-clipboard-ask read-primary-ask',
    ...
    long_text='''
Allow programs running in alatty to read and write from the clipboard. You can
...
opt('clipboard_max_size', '512',
    option_type='positive_float',
    long_text='''
The maximum size (in MB) of data from programs running in alatty that will be
...
Forward STDOUT and STDERR of the alatty process to child processes
as file descriptors 3 and 4. This is useful for debugging as it
allows child processes to print to alatty's STDOUT directly. For example,
:code:`echo hello world >&3` in a shell will print to the parent alatty's
STDOUT. When enabled, this also sets the :code:`ALATTY_STDIO_FORWARDED=3`
...
opt('+menu_map', '',
    ...
    long_text='''
Specify entries for various menus in alatty. Currently only the global menubar on macOS
...
The color of the alatty window's titlebar on Wayland systems with client
...
The color of the alatty window's titlebar on macOS. A value of
...
:code:`no`, alatty will use the macOS native :kbd:`Option+Key` to enter Unicode
...
left, right or both :kbd:`Option` keys as :kbd:`Alt`, instead. Note that alatty
itself always treats :kbd:`Option` the same as :kbd:`Alt`. This means you cannot
use this option to configure different alatty shortcuts for :kbd:`Option+Key`
vs. :kbd:`Alt+Key`. Also, any alatty shortcuts using :kbd:`Option/Alt+Key` will
take priority, so that any such key presses will not be passed to terminal
programs running inside alatty. Changing this option by reloading the config is
not supported.
'''
    )
...
opt('macos_hide_from_tasks', 'no',
    option_type='to_bool', ctype='bool',
    long_text='''
Hide the alatty window from running tasks on macOS (:kbd:`⌘+Tab` and the Dock).
...
opt('macos_quit_when_last_window_closed', 'no',
    option_type='to_bool', ctype='bool',
    long_text='''
Have alatty quit when all the top-level windows are closed on macOS. By default,
alatty will stay running, even with no open windows, as is the expected behavior
on macOS.
'''
    )
...
opt('macos_window_resizable', 'yes',
    option_type='to_bool', ctype='bool',
    long_text='''
Disable this if you want alatty top-level OS windows to not be resizable on
macOS.
'''
    )
...
font sizes. Note that in modern alatty, this option is obsolete (although still
...
this option to alter all default shortcuts that use :opt:`alatty_mod`.
'''
    )
...
to have the :option:`--hints-offset=0 <alatty +kitten hints --hints-offset>`
option applied.
'''
    )
...
argument to the program. For example::
    ...
    map alatty_mod+o pass_selection_to_program firefox

You can pass the current selection to a terminal program running in a new alatty
window, by using the :code:`@selection` placeholder::

    map alatty_mod+y new_window less @selection
'''
    )
egr()  # }}}
...
map('New window',
    'new_window alatty_mod+enter new_window',
    long_text='''
You can open a new :term:`alatty window <window>` running an arbitrary program,
for example::

    map alatty_mod+y launch mutt
...
map('New OS window',
    'new_os_window alatty_mod+n new_os_window',
    long_text='''
...
map('Close window',
    'close_window alatty_mod+w close_window',
    )
map('Close window',
...
map('Next window',
    'next_window alatty_mod+] next_window',
    )

map('Previous window',
    'previous_window alatty_mod+[ previous_window',
    )

map('Move window forward',
    'move_window_forward alatty_mod+f move_window_forward',
    )

map('Move window backward',
    'move_window_backward alatty_mod+b move_window_backward',
    )
...
map('Open the alatty command shell',
    ...
    long_text='''
Open the alatty shell in a new :code:`window` / :code:`tab` / :code:`overlay` /
:code:`os_window` to control alatty using commands.
'''
    )
...
If you want to operate on all alatty windows instead of just the current one, use
...
map('Reload alatty.conf',
    ...
    long_text='''
Reload :file:`alatty.conf`, applying any changes since the last time it was
loaded. Note that a handful of options cannot be dynamically changed and
require a full restart of alatty. Particularly, when changing shortcuts for
...
also map a keybinding to load a different config file, for example::
    ...
    map f5 load_config /path/to/alternative/alatty.conf

Note that all options from the original :file:`alatty.conf` are discarded, in
...
map('Send arbitrary text on key presses',
    ...
    add_to_default=False,
    long_text='''
You can tell alatty to send arbitrary (UTF-8) encoded text to the client program
...
:code:`application`, :code:`alatty` or a comma separated combination of them.
The modes :code:`normal` and :code:`application` refer to the DECCKM
cursor key mode for terminals, and :code:`alatty` refers to the alatty extended
