...
init_x11_startup_notification(PyObject UNUSED *self, PyObject *args) {
    ...
    if (!done) {
        ...
        const char* libnames[] = {
#if defined(_ALATTY_STARTUP_NOTIFICATION_LIBRARY)
            _ALATTY_STARTUP_NOTIFICATION_LIBRARY,
#else
...
load_libcanberra(void) {
    ...
    const char* libnames[] = {
#if defined(_ALATTY_CANBERRA_LIBRARY)
        _ALATTY_CANBERRA_LIBRARY,
#else
        "libcanberra.so",
        // some installs are missing the .so symlink, so try the full name
        "libcanberra.so.0",
...
canberra_play_loop(void *x UNUSED) {
    // canberra hangs on misconfigured systems. We dont want alatty to hang so use a thread.
    // For example: https://github.com/kovidgoyal/alatty/issues/5646
    static char buf[16];
