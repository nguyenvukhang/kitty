...
new(PyTypeObject *type, PyObject *args, PyObject UNUSED *kwds) {
    ...
    if (self != NULL) {
        ...
        if (
            self->cursor == NULL || self->main_linebuf == NULL ||
            self->alt_linebuf == NULL || self->main_tabstops == NULL
            || self->historybuf == NULL || self->color_profile == NULL
        ) {
...
#define SIDE_EFFECTS(name) case name: if (do_side_effects) set_mode_from_const(self, name, src->m##name); else dest->m##name = src->m##name; break;
    ...
    switch(mode) {
        SIMPLE_MODE(LNM)  // alatty extension
        SIMPLE_MODE(IRM)  // alatty extension
        SIMPLE_MODE(DECARM)
        SIMPLE_MODE(BRACKETED_PASTE)
        SIMPLE_MODE(FOCUS_TRACKING)
        SIMPLE_MODE(DECCKM)
...
screen_save_modes(Screen *self) {
    // alatty extension to XTSAVE that saves a bunch of no side-effect modes
...
screen_restore_modes(Screen *self) {
    // alatty extension to XTRESTORE that saves a bunch of no side-effect modes
...
screen_xtversion(Screen *self, unsigned int mode) {
    if (mode == 0) {
        write_escape_code_to_child(self, DCS, ">|alatty(" XT_VERSION ")");
    }
}
...
void screen_handle_alatty_dcs(Screen *self, const char *callback_name, PyObject *cmd) {
...
screen_is_selection_dirty(Screen *self) {
    ...
    if (self->selections.last_rendered_count != self->selections.count) return true;
    for (size_t i = 0; i < self->selections.count; i++) {
