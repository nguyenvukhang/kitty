...
def handle_cmd(
    ...
    if tuple(v)[:2] > version[:2]:
        if no_response:
            return None
        return {'ok': False, 'error': 'The alatty client you are using to send remote commands is newer than this alatty instance. This is not supported.'}
    ...
    try:
        ans = c.response_from_alatty(boss, self_window or window, PayloadGetter(c, payload))
    except Exception:
...
--to
An address for the alatty instance to control. Corresponds to the address given
to the alatty instance via the :option:`alatty --listen-on` option or the
:opt:`listen_on` setting in :file:`alatty.conf`. If not specified, the
environment variable :envvar:`ALATTY_LISTEN_ON` is checked. If that is also not
found, messages are sent to the controlling terminal for this process, i.e.
they will only work if this process is run within a alatty window.


--password
A password to use when contacting alatty. This will cause alatty to ask the user
for permission to perform the specified action, unless the password has been
accepted before or is pre-configured in :file:`alatty.conf`.
...
paths are resolved from the alatty configuration directory. Use - to read from STDIN.
Used if no :option:`kitten @ --password` is supplied. Defaults to checking for the
:file:`rc-pass` file in the alatty configuration directory.


--password-env
default=ALATTY_RC_PASSWORD
The name of an environment variable to read the password from.
Used if no :option:`kitten @ --password-file` is supplied. Defaults
to checking the environment variable :envvar:`ALATTY_RC_PASSWORD`.


--use-password
default=if-available
choices=if-available,never,always
If no password is available, alatty will usually just send the remote control command
...
class SocketIO:
    ...
    def simple_recv(self, timeout: float) -> bytes:
        ...
        if m is None:
            ...
            raise SocketClosed('Remote control connection was closed by alatty without any response being received')
...
cli_msg = (
    ...
    ' :opt:`allow_remote_control` option in :file:`alatty.conf` or use a password, for this'
    ' to work.'
).format(appname=appname)
...
def get_pubkey() -> Tuple[str, bytes]:
    raw = os.environ.get('ALATTY_PUBLIC_KEY', '')
    if not raw:
        raise SystemExit('Password usage requested but ALATTY_PUBLIC_KEY environment variable is not available')
    version, pubkey = raw.split(':', 1)
    if version != RC_ENCRYPTION_PROTOCOL_VERSION:
        raise SystemExit('ALATTY_PUBLIC_KEY has unknown version, if you are running on a remote system, update alatty on this system')
