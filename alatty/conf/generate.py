...
from alatty.conf.types import Definition, MultiOption, Option, ParserFuncType, unset
...
def generate_class(defn: Definition, loc: str) -> Tuple[str, str]:
    ...
    ki_imports: 're.Pattern[str]' = re.compile(r'\b((?:kittens|alatty).+?)[,\]]')
    ...
    for option in sorted(defn.iter_all_options(), key=lambda a: natural_keys(a.name)):
        ...
        if option.macos_defval is not unset:
            ...
            imports.add(('alatty.constants', 'is_macos'))
    ...
    t('    return ans')
    tc_imports.add(('alatty.conf.utils', 'merge_dicts'))
...
def gen_go_code(defn: Definition) -> str:
    lines = ['import "fmt"', 'import "strconv"', 'import "alatty/tools/config"', 'import "alatty/tools/utils/style"',
...
def main() -> None:
    # To use run it as:
    # alatty +runpy 'from alatty.conf.generate import main; main()' /path/to/kitten/file.py
    import importlib
    import sys
