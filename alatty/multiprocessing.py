...
# in alatty, when using the spawn launcher.
...
from .constants import alatty_exe
...
def spawnv_passfds(path: bytes, args: ArgsType, passfds: Sequence[int]) -> int:
    ...
    return orig_spawn_passfds(os.fsencode(alatty_exe()), patched_args, passfds)


def monkey_patch_multiprocessing() -> None:
    # Use alatty to run the worker process used by multiprocessing
    spawn.set_executable(alatty_exe())
