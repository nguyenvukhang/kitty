...
static void
handle_mouse_movement_in_alatty(Window *w, int button, bool mouse_cell_changed) {
    Screen *screen = w->render_data.screen;
...
HANDLER(handle_move_event) {
    ...
    bool handle_in_alatty = !in_tracking_mode || global_state.active_drag_in_window == w->id;
    if (handle_in_alatty) {
        handle_mouse_movement_in_alatty(w, button, mouse_cell_changed | cell_half_changed);
    } else {
    ...
    if (is_high_resolution) {
        ...
        *pending_scroll_pixels = pixels - s * cell_size;
        ...
        if (offset != 0) {
            ...
            // when scrolling slowly https://github.com/kovidgoyal/alatty/issues/1238
...
scroll_event(double xoffset, double yoffset, int flags, int modifiers) {
    ...
    if (!w) return;
    // Also update mouse cursor position while alatty OS window is not focused.
