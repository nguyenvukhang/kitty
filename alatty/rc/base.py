...
if TYPE_CHECKING:
    from alatty.boss import Boss as B
    from alatty.tabs import Tab as T
    from alatty.window import Window as W
    Window = W
    Boss = B
    Tab = T
else:
...
The window id of the current window is available as the :envvar:`ALATTY_WINDOW_ID` environment variable.
...
class AsyncResponder:
    ...
    def send_data(self, data: Any) -> None:
        from alatty.remote_control import send_response_to_client
...
def command_for_name(cmd_name: str) -> RemoteCommand:
    ...
    try:
        m = import_module(f'alatty.rc.{cmd_name}')
    except ImportError:
        raise KeyError(f'Unknown alatty remote control command: {cmd_name}')
...
def all_command_names() -> FrozenSet[str]:
    ...
    return frozenset({x.rpartition('.')[0] for x in filter(ok, list_alatty_resources('alatty.rc'))})
