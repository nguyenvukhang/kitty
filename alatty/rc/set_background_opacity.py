...
if TYPE_CHECKING:
    from alatty.cli_stub import SetBackgroundOpacityRCOptions as CLIOptions
...
class SetBackgroundOpacity(RemoteCommand):
    ...
    desc = (
        ...
        ' :opt:`dynamic_background_opacity` in :file:`alatty.conf`. The background opacity affects all alatty windows in a'
...
''' + '\n\n' + MATCH_WINDOW_OPTION + '\n\n' + MATCH_TAB_OPTION.replace('--match -m', '--match-tab -t')
    ...
    def response_from_alatty(self, boss: Boss, window: Optional[Window], payload_get: PayloadGetType) -> ResponseType:
        from alatty.fast_data_types import background_opacity_of, get_options
        opts = get_options()
        if not opts.dynamic_background_opacity:
            raise OpacityError('You must turn on the dynamic_background_opacity option in alatty.conf to be able to set background opacity')
