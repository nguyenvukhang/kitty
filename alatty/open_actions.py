...
protocol file
ext sh,command,tool
action launch --hold --type=os-window alatty +shebang $FILE_PATH $SHELL

# Open shell specific script files
protocol file
ext fish,bash,zsh
action launch --hold --type=os-window alatty +shebang $FILE_PATH __ext__
...
def actions_for_launch(url: str) -> Iterator[KeyAction]:
    # Custom launch actions using alatty URL scheme needs to be prefixed with `alatty:///launch/`
    if url.startswith('alatty://') and not url.startswith('alatty:///launch/'):
        return
    found = False
