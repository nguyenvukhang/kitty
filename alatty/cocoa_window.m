...
#ifndef ALATTY_USE_DEPRECATED_MACOS_NOTIFICATION_API
...
#ifdef ALATTY_USE_DEPRECATED_MACOS_NOTIFICATION_API
...
cocoa_set_app_icon(PyObject UNUSED *self, PyObject *args) {
    ...
    if (!app_path) {
        ...
        if (!bundle_path || bundle_path.length == 0) bundle_path = @"/Applications/alatty.app";
        // When compiled from source and run from the launcher folder the bundle path should be `alatty.app` in it
        if (![bundle_path hasSuffix:@".app"]) {
            NSString *launcher_app_path = [bundle_path stringByAppendingPathComponent:@"alatty.app"];
            bundle_path = @"";
            BOOL is_dir;
