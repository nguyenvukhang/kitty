...
update_modifier_state_on_modifier_key_event(GLFWkeyevent *ev, int key_modifier, bool is_left) {
    // Update mods state to be what the alatty keyboard protocol requires, as on Linux modifier key events do not update modifier bits
...
create_os_window(PyObject UNUSED *self, PyObject *args, PyObject *kw) {
    int x = INT_MIN, y = INT_MIN, window_state = WINDOW_NORMAL;
    ...
    static const char* kwlist[] = {"get_window_size", "pre_show_callback", "title", "wm_class_name", "wm_class_class", "window_state", "load_programs", "x", "y", NULL};
    if (!PyArg_ParseTupleAndKeywords(args, kw, "OOsss|OOOO", (char**)kwlist,
        &get_window_size, &pre_show_callback, &title, &wm_class_name, &wm_class_class, &optional_window_state, &load_programs, &optional_x, &optional_y)) return NULL;
    ...
    if (is_first_window) {
        ...
        // Prevents alatty from starting on Wayland + NVIDIA, sigh: https://github.com/kovidgoyal/alatty/issues/7021
    ...
    if (!apple_preserve_common_context) {
        apple_preserve_common_context = glfwCreateWindow(640, 480, "alatty", NULL, common_context);
    }
    ...
    if (!global_state.is_wayland) {
        ...
        if (temp_window == NULL) { fatal("Failed to create GLFW temp window! This usually happens because of old/broken OpenGL drivers. alatty requires working OpenGL 3.3 drivers."); }
    }
    float xscale, yscale;
    double xdpi, ydpi;
...
request_frame_render(OSWindow *w) {
    ...
    // render frame requests, see https://github.com/kovidgoyal/alatty/issues/2329
