...
@run_once
def alatty_ansi_sanitizer_pat() -> 're.Pattern[str]':
    # removes ANSI sequences generated by alatty's ANSI output routines. Not
...
def edit_config_file() -> None:
    from alatty.config import prepare_config_file_for_editing
...
def path_from_osc7_url(url: str) -> str:
    if url.startswith('alatty-shell-cwd://'):
...
def cmdline_for_hold(cmd: Sequence[str] = (), opts: Optional['Options'] = None) -> List[str]:
    ...
    return [kitten_exe(), 'run-shell', f'--shell={shell}', f'--shell-integration={ksi}', '--env=ALATTY_HOLD=1'] + list(cmd)
...
def get_custom_window_icon() -> Union[Tuple[float, str], Tuple[None, None]]:
    filenames = ['alatty.app.png']
    if is_macos:
        # On macOS, prefer icns to png.
        filenames.insert(0, 'alatty.app.icns')
