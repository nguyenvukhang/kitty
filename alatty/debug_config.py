...
from .constants import extensions_dir, is_macos, is_wayland, alatty_base_dir, alatty_exe, shell_path
...
def compare_maps(
    final: Dict[AnyEvent, str], final_alatty_mod: int, initial: Dict[AnyEvent, str], initial_alatty_mod: int, print: Print, mode_name: str = ''
) -> None:
    ei = {k.human_repr(initial_alatty_mod): v for k, v in initial.items()}
    ef = {k.human_repr(final_alatty_mod): v for k, v in final.items()}
    added = set(ef) - set(ei)
...
def compare_opts(opts: AlattyOpts, print: Print) -> None:
    ...
    compare_maps(opts.mousemap, opts.alatty_mod, default_opts.mousemap, default_opts.alatty_mod, print)
    ...
    for kmn, initial_ in default_opts.keyboard_modes.items():
        ...
        compare_maps(final, opts.alatty_mod, initial, default_opts.alatty_mod, print, mode_name=kmn)
...
def debug_config(opts: AlattyOpts) -> str:
    ...
    p(green('Paths:'))
    p(yellow('  alatty:'), os.path.realpath(alatty_exe()))
    p(yellow('  base dir:'), alatty_base_dir)
    ...
    compare_opts(opts, p)
    p()
    p(green('Important environment variables seen by the alatty process:'))
    ...
    for k in (
        'PATH LANG ALATTY_CONFIG_DIRECTORY ALATTY_CACHE_DIRECTORY VISUAL EDITOR SHELL'
        ' GLFW_IM_MODULE ALATTY_WAYLAND_DETECT_MODIFIERS DISPLAY WAYLAND_DISPLAY USER XCURSOR_SIZE'
    ).split():
        penv(k)
    for k in os.environ:
