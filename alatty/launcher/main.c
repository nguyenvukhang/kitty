...
#ifndef ALATTY_LIB_PATH
#define ALATTY_LIB_PATH "../.."
#endif
#ifndef ALATTY_LIB_DIR_NAME
#define ALATTY_LIB_DIR_NAME "lib"
#endif
...
static bool
set_alatty_run_data(RunData *run_data, bool from_source, wchar_t *extensions_dir) {
...
#undef S
    int ret = PySys_SetObject("alatty_run_data", ans);
    Py_CLEAR(ans);
...
run_embedded(RunData *run_data) {
    ...
    if (!set_alatty_run_data(run_data, from_source, NULL)) return 1;
...
read_exe_path(char *exe, size_t buf_sz) {
    ...
    while (token != NULL) {
        char q[PATH_MAX + 1] = {0};
        snprintf(q, PATH_MAX, "%s/alatty", token);
        if (safe_realpath(q, exe, buf_sz)) return true;
        token = strtok(NULL, ":");
    }
    fprintf(stderr, "alatty not found in PATH aborting\n");
    return false;
}
...
int main(int argc, char *argv[], char* envp[]) {
    ...
    if (ALATTY_LIB_PATH[0] == '/') {
        num = snprintf(lib, PATH_MAX, "%s", ALATTY_LIB_PATH);
    } else {
        num = snprintf(lib, PATH_MAX, "%s/%s", exe_dir, ALATTY_LIB_PATH);
    }

    if (num < 0 || num >= PATH_MAX) { fprintf(stderr, "Failed to create path to alatty lib\n"); return 1; }
