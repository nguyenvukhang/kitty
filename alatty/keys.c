...
on_key_input(GLFWkeyevent *ev) {
    ...
    switch(ev->ime_state) {
        ...
        case GLFW_IME_NONE:
            ...
            // On Linux this is needed by Fig integration: https://github.com/kovidgoyal/alatty/issues/5241
            update_ime_position(w, screen);
            break;
        default:
            return;
...
#define KEY_BITS 51
#define MOD_BITS 12
#if 1 << (MOD_BITS-1) < GLFW_MOD_ALATTY
...
static PyObject*
SingleKey_defined_with_alatty_mod(SingleKey *self, void UNUSED *closure) {
    if (self->defined_with_alatty_mod || (self->key.mods & GLFW_MOD_ALATTY)) Py_RETURN_TRUE;
    Py_RETURN_FALSE;
}
...
static PyGetSetDef SingleKey_getsetters[] = {
    ...
    {"defined_with_alatty_mod", (getter)SingleKey_defined_with_alatty_mod, NULL, "A bool", NULL},
    {NULL}  /* Sentinel */
};
...
static PyObject*
SingleKey_resolve_alatty_mod(SingleKey *self, PyObject *km) {
    if (!(self->key.mods & GLFW_MOD_ALATTY)) { Py_INCREF(self); return (PyObject*)self; }
    unsigned long alatty_mod = PyLong_AsUnsignedLong(km);
    if (PyErr_Occurred()) return NULL;
    SingleKey *ans = (SingleKey*)SingleKey_Type.tp_alloc(&SingleKey_Type, 0);
    if (!ans) return NULL;
    ans->key.val = self->key.val;
    ans->key.mods = (ans->key.mods & ~GLFW_MOD_ALATTY) | alatty_mod;
    ans->defined_with_alatty_mod = true;
    return (PyObject*)ans;
}
...
static PyMethodDef SingleKey_methods[] = {
    ...
    {"resolve_alatty_mod", (PyCFunction)SingleKey_resolve_alatty_mod, METH_O, ""},
    {NULL}  /* Sentinel */
};
