...
# To use fish's autoloading feature, alatty prepends the vendored integration script directory to XDG_DATA_DIRS.
...
if set -q ALATTY_FISH_XDG_DATA_DIR
    if set -q XDG_DATA_DIRS
        set --global --export --path XDG_DATA_DIRS "$XDG_DATA_DIRS"
        if set --local index (contains --index "$ALATTY_FISH_XDG_DATA_DIR" $XDG_DATA_DIRS)
    ...
    set --erase ALATTY_FISH_XDG_DATA_DIR
end
...
# "Warning: Update fish to version 3.3.0+ to enable alatty shell integration.\n"
...
function __ksi_schedule --on-event fish_prompt -d "Setup alatty integration after other scripts have run, we hope"
    functions --erase __ksi_schedule
    test -n "$ALATTY_SHELL_INTEGRATION" || return 0
    set --local _ksi (string split " " -- "$ALATTY_SHELL_INTEGRATION")
    set --erase ALATTY_SHELL_INTEGRATION
    ...
    if not contains "no-prompt-mark" $_ksi
        ...
        # With prompt marking, alatty clears the current prompt on resize,
    ...
    if not contains "no-cwd" $_ksi
        # This function name is from fish and will override the builtin one, which is enabled by default for alatty in fish 3.5.0+.
        ...
        function __update_cwd_osc --on-variable PWD --on-event fish_prompt -d "Report PWD changes to alatty"
            status is-command-substitution
            or echo -en "\e]7;alatty-shell-cwd://$hostname$PWD\a"
        end
        __update_cwd_osc
    end
    ...
    if not contains "no-sudo" $_ksi
        ...
        and not test -r "/usr/share/terminfo/x/xterm-alatty" -o -r "/usr/share/terminfo/78/xterm-alatty" 
    ...
    if test -n "$ALATTY_IS_CLONE_LAUNCH"
        ...
        if _ksi_s_is_ok "env_var"
            and test -n "$ALATTY_CLONE_SOURCE_CODE"
            set _ksi_sourced "y"
            eval "$ALATTY_CLONE_SOURCE_CODE"
        end
        if _ksi_s_is_ok "path"
            and test -r "$ALATTY_CLONE_SOURCE_PATH"
            set _ksi_sourced "y"
            source "$ALATTY_CLONE_SOURCE_PATH"
        end
        set --erase ALATTY_IS_CLONE_LAUNCH ALATTY_CLONE_SOURCE_STRATEGIES _ksi_sourced
...
function edit-in-alatty --wraps "kitten edit-in-alatty" -d "Edit the specified file in a alatty overlay window with your locally installed editor"
    kitten edit-in-alatty $argv
end

function __ksi_transmit_data -d "Transmit data to alatty using chunked DCS escapes"
...
function clone-in-alatty -d "Clone the current fish session into a new alatty window"
    set --local data
    for a in $argv
        if contains -- "$a" -h --help
            echo "Clone the current fish session into a new alatty window."
            echo
            echo "For usage instructions see: https://sw.kovidgoyal.net/alatty/shell-integration/#clone-shell"
            return
        end
