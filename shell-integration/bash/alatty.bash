...
if [[ -z "$ALATTY_SHELL_INTEGRATION" ]]; then builtin return; fi

# Load the normal bash startup files
if [[ -n "$ALATTY_BASH_INJECT" ]]; then
    builtin declare alatty_bash_inject="$ALATTY_BASH_INJECT"
    builtin declare ksi_val="$ALATTY_SHELL_INTEGRATION"
    builtin unset ALATTY_SHELL_INTEGRATION  # ensure manual sourcing of this file in bashrc does not have any effect
    builtin unset ALATTY_BASH_INJECT ENV
    if [[ -z "$HOME" ]]; then HOME=~; fi
    if [[ -z "$ALATTY_BASH_ETC_LOCATION" ]]; then ALATTY_BASH_ETC_LOCATION="/etc"; fi

    _ksi_sourceable() {
        [[ -f "$1" && -r "$1" ]] && builtin return 0; builtin return 1;
    }

    if [[ -n "$ksi_val" && "$ksi_val" != *no-sudo* && -n "$TERMINFO" && ! ( -r "/usr/share/terminfo/x/xterm-alatty" || -r "/usr/share/terminfo/78/xterm-alatty" ) ]]; then
    ...
    if [[ "$alatty_bash_inject" == *"posix"* ]]; then
        _ksi_sourceable "$ALATTY_BASH_POSIX_ENV" && {
            builtin source "$ALATTY_BASH_POSIX_ENV"
            builtin export ENV="$ALATTY_BASH_POSIX_ENV"
        }
    else
        builtin set +o posix
        builtin shopt -u inherit_errexit 2>/dev/null  # resetting posix does not clear this
        if [[ -n "$ALATTY_BASH_UNEXPORT_HISTFILE" ]]; then
            builtin export -n HISTFILE
            builtin unset ALATTY_BASH_UNEXPORT_HISTFILE
        fi

        # See run_startup_files() in shell.c in the Bash source code
        if builtin shopt -q login_shell; then
            if [[ "$alatty_bash_inject" != *"no-profile"* ]]; then
                _ksi_sourceable "$ALATTY_BASH_ETC_LOCATION/profile" && builtin source "$ALATTY_BASH_ETC_LOCATION/profile"
                for _ksi_i in "$HOME/.bash_profile" "$HOME/.bash_login" "$HOME/.profile"; do
                    _ksi_sourceable "$_ksi_i" && { builtin source "$_ksi_i"; break; }
                done
            fi
        else
            if [[ "$alatty_bash_inject" != *"no-rc"* ]]; then
                ...
                for _ksi_i in "$ALATTY_BASH_ETC_LOCATION/bash.bashrc" "$ALATTY_BASH_ETC_LOCATION/bash/bashrc" ; do
                    _ksi_sourceable "$_ksi_i" && { builtin source "$_ksi_i"; break; }
                done
                if [[ -z "$ALATTY_BASH_RCFILE" ]]; then ALATTY_BASH_RCFILE="$HOME/.bashrc"; fi
                _ksi_sourceable "$ALATTY_BASH_RCFILE" && builtin source "$ALATTY_BASH_RCFILE"
            fi
        fi
    fi
    builtin unset ALATTY_BASH_RCFILE ALATTY_BASH_POSIX_ENV ALATTY_BASH_ETC_LOCATION
    builtin unset -f _ksi_sourceable
    builtin export ALATTY_SHELL_INTEGRATION="$ksi_val"
    builtin unset _ksi_i ksi_val alatty_bash_inject
fi


if [ "${BASH_VERSINFO:-0}" -lt 4 ]; then
    builtin unset ALATTY_SHELL_INTEGRATION
    builtin printf "%s\n" "Bash version ${BASH_VERSION} too old, alatty shell integration disabled" > /dev/stderr
    builtin return
fi
...
_ksi_main() {
    ...
    IFS=" "
    for i in ${ALATTY_SHELL_INTEGRATION[@]}; do
        case "$i" in
    ...
    _ksi_debug_print() {
        # print a line to STDERR of parent alatty process
        builtin local b
    ...
    _ksi_set_mark() {
        _ksi_prompt["${1}_mark"]="\[\e]133;k;${1}_alatty\a\]"
    }
    ...
    _ksi_prompt_command() {
        # we first remove any previously added alatty code from the prompt variables and then add
        # it back, to ensure we have only a single instance
        if [[ -n "${_ksi_prompt[ps0]}" ]]; then
            PS0=${PS0//\\\[\\e\]133;k;start_alatty\\a\\\]*end_alatty\\a\\\]}
            PS0="${_ksi_prompt[ps0]}$PS0"
        fi
        if [[ -n "${_ksi_prompt[ps0_suffix]}" ]]; then
            PS0=${PS0//\\\[\\e\]133;k;start_suffix_alatty\\a\\\]*end_suffix_alatty\\a\\\]}
            PS0="${PS0}${_ksi_prompt[ps0_suffix]}"
        fi
        # restore PS1 to its pristine state without our additions
        if [[ -n "${_ksi_prompt[ps1]}" ]]; then
            PS1=${PS1//\\\[\\e\]133;k;start_alatty\\a\\\]*end_alatty\\a\\\]}
            PS1=${PS1//\\\[\\e\]133;k;start_secondary_alatty\\a\\\]*end_secondary_alatty\\a\\\]}
        fi
        if [[ -n "${_ksi_prompt[ps1_suffix]}" ]]; then
            PS1=${PS1//\\\[\\e\]133;k;start_suffix_alatty\\a\\\]*end_suffix_alatty\\a\\\]}
        fi
        if [[ -n "${_ksi_prompt[ps1]}" ]]; then
            if [[ "${_ksi_prompt[mark]}" == "y" && ( "${PS1}" == *"\n"* || "${PS1}" == *$'\n'* ) ]]; then
                ...
                # lines before the last line will be erased by alatty.
        ...
        if [[ -n "${_ksi_prompt[ps2]}" ]]; then
            PS2=${PS2//\\\[\\e\]133;k;start_alatty\\a\\\]*end_alatty\\a\\\]}
        ...
        if [[ "${_ksi_prompt[cwd]}" == "y" ]]; then
            ...
            if [[ "${_ksi_prompt[last_reported_cwd]}" != "$PWD" ]]; then
                ...
                builtin printf "\e]7;alatty-shell-cwd://%s%s\a" "$HOSTNAME" "$PWD"
            fi
        fi
    }
    ...
    if [[ "${_ksi_prompt[title]}" == "y" ]]; then
        if [[ -z "$ALATTY_PID" ]]; then
            if [[ -n "$SSH_TTY" || -n "$SSH2_TTY$ALATTY_WINDOW_ID" ]]; then
    ...
    builtin alias edit-in-alatty="kitten edit-in-alatty"
    ...
    if [ -n "${ALATTY_IS_CLONE_LAUNCH}" ]; then
        ...
        fi; if _ksi_s_is_ok "env_var" && [[ -n "${ALATTY_CLONE_SOURCE_CODE}" ]]; then
            sourced="y"
            builtin eval "${ALATTY_CLONE_SOURCE_CODE}"
        fi; if _ksi_s_is_ok "path" && [[ -r "${ALATTY_CLONE_SOURCE_PATH}" ]]; then
            sourced="y"
            builtin source "${ALATTY_CLONE_SOURCE_PATH}"
        fi
    ...
    builtin unset ALATTY_IS_CLONE_LAUNCH ALATTY_CLONE_SOURCE_STRATEGIES
}
_ksi_main
...
clone-in-alatty() {
    ...
    while :; do
        case "$1" in
            "") break;;
            -h|--help)
                builtin printf "%s\n\n%s\n" "Clone the current bash session into a new alatty window." "For usage instructions see: https://sw.kovidgoyal.net/alatty/shell-integration/#clone-shell"
                builtin return
                ;;
