...
class TestBuild(BaseTest):
    ...
    def test_exe(self) -> None:
        from alatty.constants import kitten_exe, alatty_exe, str_version
        exe = alatty_exe()
        self.assertTrue(os.access(exe, os.X_OK))
        self.assertTrue(os.path.isfile(exe))
        self.assertIn('alatty', os.path.basename(exe))
        exe = kitten_exe()
        self.assertTrue(os.access(exe, os.X_OK))
        self.assertTrue(os.path.isfile(exe))
    ...
    def test_filesystem_locations(self) -> None:
        from alatty.constants import local_docs, logo_png_file, shell_integration_dir, terminfo_dir
    ...
    def test_docs_url(self):
        ...
        from alatty.utils import docs_url
        ...
        def run_tests(p, base, suffix='.html'):
            ...
            t('#ref=conf-alatty-fonts', f'conf{suffix}#conf-alatty-fonts')
        ...
        self.ae(docs_url('#ref=issues-123'), 'https://github.com/kovidgoyal/alatty/issues/123')

    def test_launcher_ensures_stdio(self):
        import subprocess

        from alatty.constants import alatty_exe
        exe = alatty_exe()
...
os.execlp({exe!r}, 'alatty', '+runpy', 'import sys; raise SystemExit(1 if sys.stdout is None or sys.stdin is None or sys.stderr is None else 0)')
'''])
