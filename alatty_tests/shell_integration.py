...
from alatty.bash import decode_ansi_c_quoted_string
from alatty.constants import kitten_exe, alatty_base_dir, shell_integration_dir, terminfo_dir
from alatty.fast_data_types import CURSOR_BEAM, CURSOR_BLOCK, CURSOR_UNDERLINE
from alatty.shell_integration import setup_bash_env, setup_fish_env, setup_zsh_env
...
def basic_shell_env(home_dir):
    ans = {
        ...
        'ALATTY_SHELL_INTEGRATION': 'enabled',
        'ALATTY_INSTALLATION_DIR': alatty_base_dir,
...
def safe_env_for_running_shell(argv, home_dir, rc='', shell='zsh', with_kitten=False):
    ...
    if shell == 'zsh':
        ...
        if with_kitten:
            ans['ALATTY_RUNNING_BASH_INTEGRATION_TEST'] = bashrc
        else:
            setup_bash_env(ans, argv)
            ans['ALATTY_BASH_INJECT'] += ' posix'
            ans['ALATTY_BASH_POSIX_ENV'] = bashrc
...
''') as pty:
        ...
        with self.run_shell(rc=f'''PS1="{ps1}"\nexport ES="a\n b c\nd"''') as pty:
            pty.callbacks.clear()
            pty.send_cmd_to_child('clone-in-alatty')
    ...
    def test_fish_integration(self):
        ...
        completions_dir = os.path.join(alatty_base_dir, 'shell-integration', 'fish', 'vendor_completions.d')
        with self.run_shell(
            shell='fish',
            rc=f'''
set -g fish_greeting
...
''') as pty:
    ...
    ''') as pty:
        ...
        def setup_env(excluded, argv, home_dir, rc='', shell='bash', with_kitten=self.with_kitten):
            ...
            ans['ALATTY_BASH_ETC_LOCATION'] = home_dir
            ans['PS1'] = 'PROMPT $ '
            return ans
