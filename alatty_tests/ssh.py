...
from alatty.constants import is_macos, kitten_exe, runtime_dir
from alatty.fast_data_types import CURSOR_BEAM, shm_unlink
...
'''
                ...
                self.assertTrue(os.path.exists(f'{remote_home}/{tname}/78/xterm-alatty'))
                self.assertTrue(os.path.exists(f'{remote_home}/{tname}/x/xterm-alatty'))
                ...
                # isn't in contents
                contents.discard(f'{tname}/x/xterm-alatty')
                contents.discard(f'{tname}/78/xterm-alatty')
                self.ae(contents, {
                    'g.1', 'g.2', f'{tname}/alatty.terminfo', 'simple-file', 'd1/d2/x', 'd1/y', 'a/sfa', 's1', 's2',
                    '.local/share/alatty-ssh-kitten/alatty/version', '.local/share/alatty-ssh-kitten/alatty/bin/alatty',
                    '.local/share/alatty-ssh-kitten/alatty/bin/kitten'
                })
                self.ae(len(glob.glob(f'{remote_home}/{tname}/*/xterm-alatty')), 2)
...
'''
            ...
            for val in ('', 'no-rc', 'enabled no-rc'):
                for sh in self.all_possible_sh:
                    with tempfile.TemporaryDirectory() as tdir:
                        ...
                        pty.wait_till(lambda: 'alatty=fruity' in pty.screen_contents(), timeout=30)
    ...
    def check_bootstrap(self, sh, home_dir, login_shell='', SHELL_INTEGRATION_VALUE='enabled', test_script='', pre_data='', conf='', launcher='sh', home=''):
        ...
        try:
            ...
            self.assertTrue(os.path.exists(os.path.join(home_dir, '.terminfo/alatty.terminfo')))
