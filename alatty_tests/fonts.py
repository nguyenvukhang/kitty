...
from alatty.constants import is_macos, read_alatty_resource
from alatty.fast_data_types import DECAWM, get_fallback_font, sprite_map_set_layout, sprite_map_set_limits, test_render_line, test_sprite_position_for, wcwidth
from alatty.fonts.box_drawing import box_chars
from alatty.fonts.render import coalesce_symbol_maps, render_string, setup_for_testing, shape_string
...
class Rendering(BaseTest):
        ...
        def path_for_font(name):
            if name not in font_path_cache:
                with open(os.path.join(self.tdir, name), 'wb') as f:
                    ...
                    data = read_alatty_resource(name, __name__.rpartition('.')[0])
                    f.write(data)
    ...
    def test_fallback_font_not_last_resort(self):
        ...
        # macOS. See https://github.com/kovidgoyal/alatty/issues/799
