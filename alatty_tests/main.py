...
def find_all_tests(package: str = '', excludes: Sequence[str] = ('main', 'gr')) -> unittest.TestSuite:
    suits = []
    if not package:
        package = __name__.rpartition('.')[0] if '.' in __name__ else 'alatty_tests'
...
def type_check() -> NoReturn:
    from alatty.cli_stub import generate_stub  # type:ignore
...
class GoProc(Thread):
    ...
    def __init__(self, cmd: List[str]):
        ...
        env = os.environ.copy()
        env['ALATTY_PATH_TO_ALATTY_EXE'] = alatty_exe()
        self.stdout = b''
...
def run_go(packages: Set[str], names: str) -> GoProc:
    go = go_exe()
    go_pkg_args = [f'alatty/{x}' for x in packages]
    cmd = [go, 'test', '-v']
    for name in names:
        cmd.extend(('-run', name))
    cmd += go_pkg_args
...
def env_for_python_tests(report_env: bool = False) -> Iterator[None]:
    ...
    launcher_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'alatty', 'launcher')
