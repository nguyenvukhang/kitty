...
class TestConfParsing(BaseTest):
    ...
    def test_conf_parsing(self):
        from alatty.config import defaults, load_config
        ...
        from alatty.options.utils import to_modifiers
        bad_lines = []
        ...
        opts = p('alatty_mod alt')
        self.ae(opts.alatty_mod, to_modifiers('alt'))
        self.ae(next(keys_for_func(opts, 'next_layout')).mods, opts.alatty_mod)
