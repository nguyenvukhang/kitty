...
from alatty.file_transmission import Action, Compression, FileTransmissionCommand, FileType, TransmissionType, ZlibDecompressor
from alatty.file_transmission import TestFileTransmission as FileTransmission
...
class TestFileTransmission(BaseTest):
    ...
    def setUp(self):
        ...
        self.alatty_home = self.alatty_cwd = self.kitten_home = self.kitten_cwd = ''
        super().setUp()
    ...
    def run_kitten(self, cmd, home_dir='', allow=True, cwd=''):
        ...
        with set_paths(home=self.alatty_home, cwd=self.alatty_cwd):
    ...
    def setup_dirs(self):
        self.clean_tdir()
        self.alatty_home = os.path.join(self.tdir, 'alatty-home')
        self.alatty_cwd = os.path.join(self.tdir, 'alatty-cwd')
        self.kitten_home = os.path.join(self.tdir, 'kitten-home')
        self.kitten_cwd = os.path.join(self.tdir, 'kitten-cwd')
        tuple(map(os.mkdir, (self.alatty_home, self.alatty_cwd, self.kitten_home, self.kitten_cwd)))
    ...
    def test_transfer_receive(self):
        ...
        self.setup_dirs()
        self.create_src(self.alatty_home)
        ...
        os.symlink('/', os.path.join(self.alatty_home, 'sym'))
        ...
        # mirroring
        self.setup_dirs()
        self.mirror_test(self.alatty_home, self.kitten_home)
    ...
    def test_transfer_send(self):
        ...
        with self.run_kitten(['~/src', '~/src']) as pty:
            pty.wait_till_child_exits(require_exit_code=0)
        os.remove(os.path.join(self.alatty_home, 'src'))
        ...
        with self.run_kitten(['~/src', '~/sym', '~']) as pty:
            pty.wait_till_child_exits(require_exit_code=0)
        os.remove(os.path.join(self.alatty_home, 'src'))
        os.remove(os.path.join(self.alatty_home, 'sym'))
        ...
        # mirroring
        self.setup_dirs()
        self.mirror_test(self.kitten_home, self.alatty_home, prefix='~/')
