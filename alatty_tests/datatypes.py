...
from alatty.config import build_ansi_color_table, defaults
...
from alatty.fast_data_types import Cursor as C
from alatty.rgb import to_color
from alatty.utils import is_ok_to_read_image_file, is_path_in_temp_dir, sanitize_title, sanitize_url_for_dispay_to_user, shlex_split_with_positions
...
class TestDataTypes(BaseTest):
    ...
    def test_utils(self):
        ...
        fifo = os.path.join(tempfile.gettempdir(), 'test-alatty-fifo')
        os.mkfifo(fifo)
    ...
    def test_single_key(self):
        from alatty.fast_data_types import GLFW_MOD_ALATTY, GLFW_MOD_SHIFT, SingleKey
        for m in (GLFW_MOD_ALATTY, GLFW_MOD_SHIFT):
            s = SingleKey(mods=m)
            self.ae(s.mods, m)
        ...
        self.ae(repr(SingleKey(key=23)._replace(key=-1, mods=GLFW_MOD_ALATTY)), f'SingleKey(mods={GLFW_MOD_ALATTY})')
    ...
    def test_notify_identifier_sanitization(self):
        from alatty.notify import sanitize_identifier_pat
        self.ae(sanitize_identifier_pat().sub('', '\x1b\nabc\n[*'), 'abc')

    def test_bracketed_paste_sanitizer(self):
        from alatty.utils import sanitize_for_bracketed_paste
